...
RankCorrelation::RankCorrelation(const vector<double> &v1,
 ...
 */
void RankCorrelation::setRanksAndTies(const vector<double> &v,
                                      vector<double> &r, vector<double> &t,
                                      double &sumt) {
...
double RankCorrelation::getKendalls(const bool &normappr) {
  ...
  cout << "|to_normal( tau )| = "
       << abs(tau / sqrt(2 * (2 * n + 5) / (9 * n * (n - 1)))) << endl;
  if (normappr) {
  ...
  for (size_t i = 0; i < ranks.size(); ++i) {
    ...
    for (size_t j = i; j < ranks.size(); ++j) {
      ...
      if (val > rank2)
        ...
        double term = (static_cast<double>(numties - myorder - 1)) /
                      static_cast<double>(numties);
        C += term;
      }
    }
    cout << ranks[i].getFirst() << " " << ranks[i].getSecond() << " " << C
         << endl;
    N += C;
  }
  return N;
