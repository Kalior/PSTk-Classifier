...
class GeneRecord {
 public:
  GeneRecord() {}
  GeneRecord(const string &name, const char &dir, const int &start,
             const int &stop)
      : name_(name), direction_(dir), startkoord_(start), stopkoord_(stop) {}
  ~GeneRecord() {}
  ...
  bool isMyName(const string &pat) {
    if (pat == name_)
      return true;
    else
      return false;
  ...
  void Print() {
    cout << name_ << " " << direction_ << " " << startkoord_ << " "
         << stopkoord_ << endl;
  }
