      ...
      for (size_t i = 0; i < tmp_str.size(); ++i) {
        if (i > 0) {
          if (tmp_str[i - 1] != '=' && tmp_str[i] == '"') {
            ...
            if (currname == "")
              proteinname_.push_back(notename);
            else
              proteinname_.push_back(currname);
            protseq_ = "";
            break;
          }
        }
...
string GeneBankRecord::getSequence(const string &pat, bool allow_complement) {
  ...
  for (size_t i = 0; i < genes_.size(); ++i) {
    //      cout << genes_[ i ].getStartKoord() << " " << genes_[ i
    //      ].getStopKoord() << endl;
    if (genes_[i].isMyName(pat)) {
      if (genes_[i].getDirection() == 'R')
        if (allow_complement) iscomplement = true;
      for (size_t j = genes_[i].getStartKoord() - 1;
           j < genes_[i].getStopKoord(); ++j) {
        res += nucseq_[j];
      }
    }
  }
...
void GeneBankRecord::printNonCoding() {
  ...
  for (size_t i = 0; i < genes_.size(); ++i) {
    ...
    if (prev_idx < genes_[i].getStartKoord()) {
      cout << "> " << prev_idx + 1 << " to " << genes_[i].getStartKoord() - 1
           << endl;
      isendl = false;
    }
    for (size_t j = prev_idx, k = 0; j < genes_[i].getStartKoord() - 1;
         ++j, ++k) {
      isendl = false;
      cout << nucseq_[j];
      if ((k + 1) % 100 == 0) {
...
void GeneBankRecord::printAllSequences(const bool &allow_complement) {
  ...
  for (size_t i = 0; i < genes_.size(); ++i) {
    if (genes_[i].getDirection() == 'R')
      if (allow_complement) iscomplement = true;
    for (size_t j = genes_[i].getStartKoord() - 1; j < genes_[i].getStopKoord();
         ++j) {
      res += nucseq_[j];
    }
    if (iscomplement) makeComplement(res);
    cout << ">";
    if (genes_[i].getName() != "")
      cout << genes_[i].getName() << endl;
    else
      cout << genes_[i].getNote() << endl;
    bool isendl = false;
...
void GeneBankRecord::printRecords() {
  for (size_t i = 0; i < genes_.size(); ++i) {
    cout << genes_[i].getStartKoord() << "\t" << genes_[i].getStopKoord()
         << "\t" << genes_[i].getName() << "\t" << genes_[i].getDirection()
         << "\n";
  }
}
 ...
 * clears attributes
 */
void GeneBankRecord::resetAttributes() { nucseq_ = ""; }
