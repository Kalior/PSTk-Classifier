...
void Fasta::setSeed(const int &seed) { prand_->randomize(seed); }

int Fasta::getLength() {
  int length = 0;
  if (fin_.peek() == EOF) return length;
  if (fin_.get() != '>') {
    cerr << "This is no fasta file!\n";
  }
  ...
  while (fin_.peek() != '>' && fin_.peek() != EOF) {
    ...
    for (int i = 0; buff[i] != '\0' && MAXLINELENGTH; ++i) {
      if (buff[i] == 'A' || buff[i] == 'a' || buff[i] == 'C' ||
          buff[i] == 'c' || buff[i] == 'G' || buff[i] == 'g' ||
          buff[i] == 'T' || buff[i] == 't')
        ++length;
    }
  }
...
string Fasta::getSequenceFromRandomIntervall(const int &len, double &start) {
  ...
  if (start >= 0) {
    int i = 0;
    // previous start is not allowed to be in the same intervall, ie. start !=
