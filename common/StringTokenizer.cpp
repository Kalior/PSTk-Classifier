...
bool StringTokenizer::hasMoreTokens() { return (tokenStr.length() > 0); };

string StringTokenizer::nextToken() {
  if (tokenStr.length() == 0) return "";

  string tStr = "";
  int pos = tokenStr.find(delim, 0);
...
string StringTokenizer::nextToken(string delimiter) {
  ...
  string tStr = "";
  int pos = tokenStr.find(delimiter, 0);

  if (pos != string::npos) {
    tStr = tokenStr.substr(0, pos);
    tokenStr =
        tokenStr.substr(pos + delimiter.length(), tokenStr.length() - pos);
...
string StringTokenizer::remainingString() { return tokenStr; };
