...
class myPair {
 public:
  ...
  bool operator<(const myPair<U, V> &a) const { return second < a.second; }
};

RandomDistribution::RandomDistribution(const vector<T> &x_value,
                                       const vector<T> &y_frequency) {
...
// cumdist_
{
  ...
  for (/****/; i < PRECISION; ++i, ++j) {
    while ((i++) * steplength < cumdist_[j]) {
      ...
      // if( i%10 == 0 ) cout << i << " " << counter << " " <<
      // indexmapping_.size()<< " " << cumdist_[ cumdist_.size() -1 ] << endl;
    }
    ++counter;
  }
}
