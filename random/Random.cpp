...
bool rlu_random::CheckPrecission() {
  long i;
  for (_rmax = 1, i = 0; i < _bitsize; i++) _rmax *= 2;
...
double rlu_random::gamma(double a) {
  ...
  do {
    r = gamma(k);
    g = r + a - k;
  } while ((g < 1e-10) || (flat() > exp(a * log(g / a) - k * log(r / k))));
  return g;
}
...
//     cout << "Initiation from file failed. rlu_random object unchanged." <<
//     endl; return in;
//   }
//   in.get(c);
//   in.get(dump,100,'\n');
//   if(strcmp("# bitsize, seed, count, i1, i2, rr97, rr98, rr99,
//   rr[0-96]",dump)){
//     cout << "Initiation from file failed. rlu_random object unchanged." <<
//     endl; return in;
//   }
