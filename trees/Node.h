...
class Node {
 public:
   ...
   */
  void setNextSymbolProb(const int &i, const double &p) {
    assert(i < nextSymbolProb_.size());
    nextSymbolProb_[i] = p;
  }
   ...
   */
  void setNextSymbolCount(const int &i, const double &p) {
    assert(i < nextSymbolCount_.size());
    nextSymbolCount_[i] = p;
  }
   ...
   */
  double getNextSymbProb(const int &i) const {
    assert(i < nextSymbolProb_.size());
    return nextSymbolProb_[i];
  }
   ...
   */
  double getNextSymbCount(const int &i) const {
    assert(i < nextSymbolCount_.size());
    return nextSymbolCount_[i];
  }
