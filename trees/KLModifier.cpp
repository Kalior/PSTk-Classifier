...
void KLModifier::prune(const double &cutoff) {
  ...
  if (K < 0) {
    K = 0.5 * StatisticalTables::quantileChiSquare(head_->getAlphabetSize() - 1,
                                                   0.05);
    cerr << "No cutoff set: K = " << K << " is recommended!\n";
    cerr << "Re-run but specify cutoff with \"-nc " << K << "\n";
    exit(1);  // Occassionally generates segmentation-fault if re-define cutoff
              // (DD 2005-11-01)
  }
  ...
  while (head_->getNumberParameters() > num) {
    ...
    if (!hasRemoved) {
      ...
      if (count > TreeModifier::MAXITERATION) {
        cerr << "In KLModifier: Prune did not converge for " << num
             << " parameters!\n";
        break;
      }
    }
...
void KLModifier::prune(Node *pNode, const double &cutoff, bool &hasRemoved) {
  for (int i = 0; i < pNode->getAlphabetSize(); ++i) {
    ...
    if (next->isLeaf()) {
      ...
      // 	  cout << pNode->getString() << " " << tree_->getCount(
      // pNode->getString() ) << " "
      // 	       << next->getString() << " " << tree_->getCount(
      // next->getString() ) << " "
      // 	       << delta << " " << cutoff << endl;
      if (delta <= cutoff) {
        delete next;
        ...
        //	      cout << "DO NOT DELETE NODE: " << next->getString() << " "
        //<< delta << endl;
      }
    } else {
...
bool KLModifier::prune(Node *pNode, const double &cutoff, bool &hasRemoved,
                       const int &num) {
  for (int i = 0; i < pNode->getAlphabetSize(); ++i) {
    ...
    if (next->isLeaf()) {
      ...
      //	  cout << delta << endl;
      if (delta <= cutoff) {
        if (Node::getNumberNodes() <= 5)
          return false;  // Avoid choosing a worse model with same number
                         // parameters
        delete next;
        next = NULL;
        pNode->setNode(NULL, i);
