...
trees::Tree *TreeFactory::getObject(Parameters &params) {
  ...
  switch (params.GetIntParam("-model")) {
    ...
    {
      trees::Tree *myTree =
          new trees::PstTree(alphabet_, params.GetIntParam("-kmax"));
      ...
      if (method != "") {
        //     * \exception see class Exception.h in common, if wrong method
        //     specified
        if (method == "PS") {
          ...
          cerr << " No such TreeModifier: " << method
               << ", none will be used!\n";
          // throw common::Exception( "No such modifier...",
