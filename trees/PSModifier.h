 ...
 * \f$
 * <br><br>
 * denote the frequency of occurrence of the word v of length \f$\ell \leq n\f$
 * in \f$x_1^n\f$. <br><br> Using these frequencies, we can form the
 * <k>empirical transition probabilities</k>: for \f$v \in \Gamma^{\ell}$ and $a
 * \in \Gamma\f$,<br><br> \f$ \tilde{p}_{v,a} := N_x(va)/N_x(v) \f$<br><br>
 *
 * The basic observation of Peres and Shields is that if the source is of true
 * order k, then <br> for any \f$v \in \Gamma^{\ell}\f$, the empirical
 * probabilities \f$\tilde{p}_{v,a}\f$ should be very close to the empirical
 * probabilities <br> \f$\tilde{p}_{\tau_k(v),a}\f$, where \f$\tau_k(v)\f$
 * denotes the \f$k\f$-suffix of \f$v\f$ i.e. \f$\tau_k(v) := (v_{\ell-k+1},
 * \cdots,v_{\ell})\f$.
 *
 * To quantify this, introduce the <k>Peres--Shields Fluctuation</k> function
 * <br><br> \f$ \label{eq:fluc-fun} \Delta^k_x(v) := \max_{a \in \Gamma}
 * \left|N_x(va) - \frac{N_x(\tau_k(v)a)}{N_x(\tau_k(v))} N_x(v)\right|.
 * \f$<br><br>
 * If the true order is k or less, one expects this fluctuation to be ``small'',
 * otherwise ``large''.
 *
